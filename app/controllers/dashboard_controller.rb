class DashboardController < ApplicationController
  ...
  def show
    if params[:filter] == "open"
      @posts = Post.where(created_by: current_user).open.order('created_at ASC')
      ...
      @posts = Post.where(created_by: current_user).completed.completedorder('created_at ASC')
      #@posts.zip(Post.where(buyer: current_user).completed).flatten.compact      
    elsif params[:filter] == "all"
      @posts = Post.where(created_by: current_user).order('created_at ASC')
